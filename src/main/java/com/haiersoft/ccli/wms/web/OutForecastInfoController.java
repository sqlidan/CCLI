 ...
 */
@Controller
@RequestMapping("wms/outforecastinfo")
public class OutForecastInfoController extends BaseController{
	...
	public Map<String, Object> getData(HttpServletRequest request,@PathVariable("forId") String forId) {
		Page<BisOutForecastInfo> page = getPage(request);
		List<PropertyFilter> filters = PropertyFilter.buildFromHttpRequest(request);
		
		PropertyFilter filter = new PropertyFilter("EQS_forId", forId);
		filters.add(filter);
		page = outForecastInfoService.search(page, filters);
	...
	public String createForm(@PathVariable("forId") String forId,Model model) {
		...
		model.addAttribute("bisOutForecastInfo", new BisOutForecastInfo());
		model.addAttribute("action", "create");
		return "wms/outforecast/outForecastInfoAdd";
	}
	...
	@ResponseBody
	public String create(@Valid BisOutForecastInfo bisOutForecastInfo,Model model, HttpServletRequest request) {
		outForecastInfoService.save(bisOutForecastInfo);
		return "success";
	}
	...
	public String updateForm(@PathVariable("id") Integer id, Model model) {
		model.addAttribute("bisOutForecastInfo", outForecastInfoService.get(id));
	...
	@ResponseBody
	public String update(@Valid @ModelAttribute @RequestBody BisOutForecastInfo bisOutForecastInfo,Model model) {
		outForecastInfoService.update(bisOutForecastInfo);
		return "success";
	}
	...
	public String deleteContractInfo(@PathVariable("ids") List<Integer> ids) {
		for(int i=0;i<ids.size();i++){
			outForecastInfoService.delete(ids.get(i));
		}
		return "success";
	}
	...
	@ResponseBody
	public String createCopy(@Valid BisOutForecastInfo bisOutForecastInfo,Model model) {
		outForecastInfoService.save(bisOutForecastInfo);
		return "success";
	}
