...
public class ScmTaskThirdController implements Job {

    private static final Logger logger = LoggerFactory.getLogger(ScmTaskTwoController.class);
    ...
    public static final String outboundAddUrl = "https://apiplat.sdland-sea.com/scm/outboundAdd";//增量推送出库明细
    ...
    public void sortData() throws IOException, ParseException {
        ...
        if (outboundAddList!=null && outboundAddList.size() > 0 && outboundAddList.get(0)!=null) {
            Map<String,String> scmDictAllMap = converDictListToMap(scmDictAllList);
            for (Map<String,Object> forMap:outboundAddList) {
...
//                forMap.put("productType","XXXX");//String-货种，字典项编码-是-多个逗号分隔。字典项：product_type
            ...
            outboundAddList = new ArrayList<>();
        }


        //执行发送
        execute(outboundAddList);

        return;
    }

    //执行发送
    public void execute(List<Map<String,Object>> outboundAddList) {
        SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date date = new Date();
        //增量推送出库明细
        Map<String, Object> params3 = new HashMap<String, Object>();
        UUID uuid3 = UUID.randomUUID();
        String str3 = uuid3.toString().replace("-", "");
        params3.put("requestId", str3);
        params3.put("requestDate", sdf.format(date));
        params3.put("warehouseName", warehouseName);
        params3.put("warehouseCode", warehouseCode);
        params3.put("key", key);
        params3.put("data", outboundAddList);
        logger.info("增量推送出库明细参数：" + JSON.toJSONString(params3));
        String bodys3 = HttpUtil.createPost(outboundAddUrl)
                .body(JSON.toJSONString(params3))
                .execute()
                .body();
