...
@Repository
public class CustomsInStockReportDao extends HibernateDao<CustomsInStock, String> {
	
	/**
	 * 
	 * @author cuij
	 * @Description: 分页查询
	 * @date 2016年6月21日15:05:53 
	 * @param page
	 * @param expenseScheme
	 * @return
	 * @throws
	 */
	public Page<CustomsInStock> searchStockReport(Page<CustomsInStock> page, CustomsInStock customsStock,double ntaxe){
		...
		StringBuffer sql = new StringBuffer(""
                ...
                + " null as verifinum, /*核销单号*/ "
                + " d.TRADE_TYPE as tradetype, /*贸易类型*/"
                ...
                + " where d.in_out_sign='1' ");
		if(null!=customsStock.getSearchStrTime() && !"".equals(customsStock.getSearchStrTime())){
			sql.append(" and d.declare_time >= :searchStrTime ");
			params.put("searchStrTime", customsStock.getSearchStrTime());
		}
		if(null!=customsStock.getSearchEndTime() && !"".equals(customsStock.getSearchEndTime())){
			sql.append(" and d.declare_time <= :searchEndTime ");
			params.put("searchEndTime", DateUtils.addMonth(customsStock.getSearchEndTime(), 1));
		}
		
		//查询对象属性转换
		Map<String, Object> parm = new HashMap<String, Object>();
		parm.put("itemNum", String.class);//项号
		parm.put("CurrencyType", String.class);//币种
		parm.put("cargoName", String.class);//品名
		parm.put("declareTime", Date.class);//申报日期
		parm.put("cdNum", String.class);//报关号
		parm.put("netWeight", Double.class);//净重（KG）
		parm.put("totalPrices", Double.class);//货值（美元）
		parm.put("verifinum", String.class);//核销单号
		parm.put("tradetype", String.class);//贸易类型
