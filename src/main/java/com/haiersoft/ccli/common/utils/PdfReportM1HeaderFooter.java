...
public class PdfReportM1HeaderFooter extends PdfPageEventHelper {
	 ...
	 */
	public Rectangle pageSize = PageSize.A3;
	...
	public void setHeadert(String headert) {
...
//		try {
//			image = Image.getInstance("C:\\123\\tpdf2.jpg");
...
//        image.setAbsolutePosition(12, 300);
	...
	public void onEndPage(PdfWriter writer, Document document) {
		...
		try {
			...
			if (fontDetail == null) {
				fontDetail = new Font(bf, presentFontSize, Font.UNDERLINE+Font.BOLDITALIC);// 数据体字体
			}
		...
		ColumnText.showTextAligned(writer.getDirectContent(),
				Element.ALIGN_CENTER, new Phrase(headert, new Font(bf, presentFontSize, Font.BOLDITALIC)),
				(document.right() + document.left())/2, document.top() +50, 0);
		ColumnText.showTextAligned(writer.getDirectContent(),
				Element.ALIGN_LEFT, new Phrase(header, fontDetail),
				document.left()+20, document.top()+20, 0);
  		}
		// 2.写入前半部分的 第 X页/共
		int pageS = writer.getPageNumber();
		String foot1 = "第 " + pageS + " 页 /共";
		Phrase footer = new Phrase(foot1, new Font(bf, presentFontSize, Font.NORMAL));
