...
<div data-options="region:'south',split:true,border:false" title="入库报检货物信息"  style="height:200px">
...
function jiaoyan(){
	$('#billNum').validatebox({    
	    	...
	    	validType:{
	    	length:[4,20],
	    	remote:["${ctx}/wms/ciq/checkbillnum/"+$("#cid").val(),"billNum"]
	    }
	}); 
}
...
//清空
function clearIt(){
	window.parent.mainpage.mainTabs.addModule('入库报检管理','wms/ciq/add');
}
...
function gridDG(){
	...
	dg=$('#dg').datagrid({    
		method: "GET",
	    url:'${ctx}/wms/ciqinfo/json/'+ciqId, 
	    fit : true,
		fitColumns : true,
		border : false,
		striped:true,
...
function addInfoT(){
	...
	d=$("#dlg").dialog({   
	    ...
	    width: 380,    
	    height: 380,    
	    href:'${ctx}/wms/ciqinfo/create/'+cid+'/'+ciqNum,
	    maximizable:true,
	    modal:true,
	    buttons:[{
			text:'确认',
...
function editInfo(){
	...
	d=$("#dlg").dialog({   
	    ...
	    width: 380,    
	    height: 380,    
	    href:'${ctx}/wms/ciqinfo/update/'+row.id,
	    maximizable:true,
	    modal:true,
	    buttons:[{
			text:'确认',
...
function delInfo(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type: 'get',
				url: "${ctx}/wms/ciqinfo/deleteinfo/" + ids,
				success: function(data){
...
d=$("#dlg").dialog({   
	    ...
	    width: 380,    
	    height: 340,    
	    href:'${ctx}/wms/ciqinfo/copy/'+row.id,
	    maximizable:true,
	    modal:true,
	    buttons:[{
			text:'增加',
...
function exportm(){
	...
	$.ajax({
		...
		success: function(data){
			if(data == "success"){
				parent.$.easyui.messager.show({title: "操作提示", msg: "请先保存入库报检单！", position: "bottomRight"});
				return;
			}else{
				var url = "${ctx}/wms/ciq/exportwith/"+cid;
	...
	$.ajax({
		...
		success: function(data){
			if(data == "success"){
				...
				window.parent.mainpage.mainTabs.addModule('入库报检单（带货物信息）打印','wms/ciq/printInfo/' + cid);
			}
		}
	});	
