...
<div data-options="region:'center'" title="入库报检单总览" style="overflow-y:auto">
  ...
  <!--    	<select id="searchStock" name="filter_LIKES_clientName" class="easyui-combobox" data-options="width:150,prompt: '客户名称'" >
		...
		<shiro:hasPermission name="wms:ciq:add">
	      	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-add" plain="true" onclick="window.parent.mainpage.mainTabs.addModule('入库报检管理','wms/ciq/add')">添加</a>
	      	<span class="toolbar-item dialog-tool-separator"></span>
      	</shiro:hasPermission>
      	<shiro:hasPermission name="wms:ciq:delete">
      		 <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" data-options="disabled:false" onclick="del()">删除</a>
       		<span class="toolbar-item dialog-tool-separator"></span>
      	</shiro:hasPermission>
      	<shiro:hasPermission name="wms:ciq:update">
	      	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="update()">修改</a>
	      	<span class="toolbar-item dialog-tool-separator"></span>
      	</shiro:hasPermission>
      	<shiro:hasPermission name="wms:ciq:export">
        	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-page-excel" plain="true" onclick="exportExcel()">导出</a>
        	<span class="toolbar-item dialog-tool-separator"></span>
        </shiro:hasPermission>
        <shiro:hasPermission name="wms:ciq:exportinfo">
   ...
   <!--   
 <shiro:hasPermission name="wms:ciq:wc">
...
<div data-options="region:'south',split:true,border:false" title="入库报检货物信息汇总"  style="height:200px">
	...
	dg=$('#dg').datagrid({    
	    ...
	    columns:[[   
	        ...
	        {field:'ciqCode',title:'报检单号',sortable:true,width:20},    
 	        ...
 	        {field:'clientName',title:'客户',sortable:true,width:20},   
...
function update(){
	var row = dg.datagrid('getSelected');
	if(rowIsNull(row)) return;
		window.parent.mainpage.mainTabs.addModule('入库报检单修改','wms/ciq/update/' + row.id);
}
...
function exportInfo(){
	var row = dg.datagrid('getSelected');
	if(rowIsNull(row)){
	   parent.$.messager.show({ title : "提示",msg: "请选择一条入库报检单数据！", position: "bottomRight" });
	   return;
	}
	var url = "${ctx}/wms/ciq/exportwith/"+row.id;
...
function print(){
	...
	window.parent.mainpage.mainTabs.addModule('入库报检单打印','wms/ciq/print/' + ids);
}
...
function printInfo(){
	...
	window.parent.mainpage.mainTabs.addModule('入库报检单（带货物信息）打印','wms/ciq/printInfo/' + row.id);
}
