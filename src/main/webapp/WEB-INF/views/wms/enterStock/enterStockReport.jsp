	...
	<div>
		<form id="searchFrom" action="" method="post">
			<input type="hidden" name="ntype" id="ntype"/>
      	    <input type="text" id="searchItemNum" name="searchItemNum" class="easyui-validatebox" data-options="width:250,prompt: '提单号（多个提单号请用英文的逗号分割）'"/>
      	    <input type="text" name="searchCunNum" class="easyui-validatebox" data-options="width:150,prompt: '箱号'"/>
      	    <select id="searchStock" name="searchStockIn" class="easyui-validatebox" data-options="width:150,prompt: '存货方'" ></select>
      	    <input type="text" name="searchSKUNum" class="easyui-validatebox" data-options="width:150,prompt: 'SKU'"/>
      	    <!-- <input type="text" name="searchLinkId" class="easyui-validatebox" data-options="width:150,prompt: '入库联系单号'"/> -->
	        <input type="text" name="searchStrTime" class="easyui-my97" datefmt="yyyy-MM-dd HH:mm:ss" data-options="width:150,prompt: '入库日期起'"/>
	        <input type="text" name="searchEndTime" class="easyui-my97" datefmt="yyyy-MM-dd HH:mm:ss" data-options="width:150,prompt: '入库日期止'"/>
        ...
        <shiro:hasPermission name="wms:enterStock:japanp">
        ...
        <shiro:hasPermission name="wms:enterStock:otep">
...
$(function(){   
	//客户
	 ...
	 //加载页面数据表格
       dg = $('#dg').datagrid({
           method: "get",
           url: '${ctx}/wms/enterStock/jsonEnterReport',//
           fit: true,
           fitColumns: false,
           border: false,
           //idField: 'linkId',
...
function report(ntype){
	if ($("#searchFrom").form('validate')) {
		$("#ntype").val(ntype);
		var url = "/ccli/wms/enterStock/report";
...
function reportpdf(ntype){
	if ($("#searchFrom").form('validate')) {
		$("#ntype").val(ntype);
	 	var url = "/ccli/wms/enterStock/reportpdf";
	 	$("#searchFrom").attr("action",url).submit();
	}
}
