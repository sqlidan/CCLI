...
<div data-options="region:'center'" title="入库预报单列表" style="overflow-y:auto">
		...
		<shiro:hasPermission name="wms:forecast:add">
	      	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-add" plain="true" onclick="window.parent.mainpage.mainTabs.addModule('入库预报单管理','wms/forecast/create')">添加</a>
	      	<span class="toolbar-item dialog-tool-separator"></span>
      	</shiro:hasPermission>
...
function gridDG(){	
	dg=$('#dg').datagrid({    
		...
		singleSelect:true,
	    columns:[[   
	    	{field:'forId',title:'入库预报单ID',sortable:true,width:100},  
	        {field:'billNum',title:'提单号',sortable:true,width:100},    
 	        ...
 	        {field:'ciqDeclarationUnit',title:'报检公司',sortable:true},
...
function update(){
	...
	window.parent.mainpage.mainTabs.addModule('入库预报单修改','wms/forecast/update/' + row.forId);
}
...
function del(){
	var row = dg.datagrid('getSelected');
	if(rowIsNull(row)){
	   parent.$.messager.show({ title : "提示",msg: "请选择一条入库预报单数据！", position: "bottomRight" });
	   return;
	}
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/wms/forecast/delete/"+row.forId,
...
function print(){
	...
	window.parent.mainpage.mainTabs.addModule('入库预报单打印','wms/forecast/print/' + row.forId);
}

//导出
function exportExcel(){
 	var url = "${ctx}/wms/forecast/export";
...
function addInfo(){
	...
	d=$("#dlg").dialog({   
	    ...
	    width: 380,    
	    height: 380,    
	    href:'${ctx}/wms/forecastinfo/create/'+row.forId,
	    maximizable:true,
	    modal:true,
	    buttons:[{
			text:'确认',
...
function delInfo(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type: 'get',
				url: "${ctx}/wms/forecastinfo/deleteinfo/" + row.id,
				success: function(data){
...
function submitCd(){
	...
	$.ajax({
				type: 'get',
				url: "${ctx}/wms/forecast/customs/" + row.forId,
				success: function(data){
					if(data == "success"){
						successTip(data, dg);
					}else{
...
function submitCiq(){
	...
	$.ajax({
				type: 'get',
				url: "${ctx}/wms/forecast/ciq/" + row.forId,
				success: function(data){
					if(data == "success"){
						successTip(data, dg);
					}else{
