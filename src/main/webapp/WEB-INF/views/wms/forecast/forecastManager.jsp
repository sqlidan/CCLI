...
<div data-options="region:'center'">
	...
	<!--	<shiro:hasPermission name="wms:customsm:add">
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-add" plain="true" onclick="clearIt()">添加</a>
	  	...
	  	<shiro:hasPermission name="wms:forecastma:save">
	    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-disk" plain="true" data-options="disabled:false" onclick="submitForm()">保存</a>
	   	...
	   	<shiro:hasPermission name="wms:forecastma:delete">
	    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="deleteIt()">删除</a>
	    <span class="toolbar-item dialog-tool-separator"></span>
	    </shiro:hasPermission>
	     <shiro:hasPermission name="wms:forecastma:submitCD">
        	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-user-edit" plain="true" onclick="submitCd()">生成入库报关单</a>
        	<span class="toolbar-item dialog-tool-separator"></span>
        </shiro:hasPermission>
        <shiro:hasPermission name="wms:forecastma:submitCIQ">
        	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-user-edit" plain="true" onclick="submitCiq()">生成入库报检单</a>
        	<span class="toolbar-item dialog-tool-separator"></span>
        </shiro:hasPermission>
	</div>
	<table class="formTable" >
		<tr>
			<td>入库预报单ID</td>
			<td>
				<input type="text" id="forId" name="forId"  class="easyui-validatebox" value="${bisForecast.forId}" data-options="width:180"  readonly style="background:#eee">
			</td>
			<td>客户名称：</td>
				<td>
		...
		<tr>
			<td>提单号：</td>
				<td>
					<input id="billNum" name="billNum" type="text" class="easyui-validatebox"  data-options="width:180, required:'required'" maxlength="17"  value="${bisForecast.billNum}">
				</td>
			<td>箱量：</td>
				<td>
				<input type="text" id="ctnCont" name="ctnCont" class="easyui-validatebox"  data-options="width:180" maxlength="10" onkeyup="ischeckNum(this)"  value="${bisForecast.ctnCont}">
				</td>
			<td>贸易方式：</td>
				<td>
			...
			<td>报关单号：</td>
				<td>
				<input type="text" id="cdNum" name="cdNum" class="easyui-validatebox"  data-options="width:180" maxlength="25"   value="${bisForecast.cdNum}">
			</td>
		</tr>
		<tr>
			<td>客服：</td>
				<td>
				<input id="customerService" name="customerService" class="easyui-validatebox"  data-options="width:180"  value="${bisForecast.customerService}" style="background:#eee"  readonly>
				</td>
			<td>修改日期</td>
			<td>
				<input id="updateTime" name="updateTime" class="easyui-my97" data-options="width:180" datefmt="yyyy-MM-dd HH:mm:ss" value="<fmt:formatDate value="${bisForecast.updateTime}" pattern="yyyy-MM-dd HH:mm:ss" />" disabled/> 
			</td>
			<td>创建日期</td>
			<c:choose>
				...
				<c:otherwise>
				<td>
					<input id="createTime" name="createTime" class="easyui-my97" data-options="width:180" datefmt="yyyy-MM-dd HH:mm:ss"  value="<fmt:formatDate value="${bisForecast.createTime}" pattern="yyyy-MM-dd HH:mm:ss" />" disabled/> 
				</td>
				</c:otherwise>
			</c:choose>
			<td>报检单号：</td>
				<td>
				<input type="text" id="ciqNum" name="ciqNum" class="easyui-validatebox"  data-options="width:180" maxlength="25"   value="${bisForecast.ciqNum}">
			</td>
		</tr>
		<tr>
				<td>备注：</td>
...
if($("#forId").val() == "" ){
  ...
  $.ajax({
		type : "POST",
		url : "${ctx}/wms/forecast/getNewForId",
		dataType : "text",
...
function selectAjax(){
	//报关公司
	  	var getorg='${bisForecast.declarationUnitId}';
	   $('#declarationUnitId').combobox({
	   method:"GET",
	   url:"${ctx}/base/client/getClientAll?setid=${bisForecast.declarationUnitId}",
	   valueField: 'ids',
   	...
   	//报检公司
	  	var getorgb='${bisForecast.ciqDeclarationUnitId}';
	   $('#ciqDeclarationUnitId').combobox({
	   method:"GET",
	   url:"${ctx}/base/client/getClientAll?setid=${bisForecast.ciqDeclarationUnitId}",
	   valueField: 'ids',
	   textField: 'clientName',
	   mode:'remote',
	   onLoadSuccess:function(){
   	...
   	//客户
 	var getstockId='${bisForecast.clientId}';
	   $('#clientId').combobox({
	   method:"GET",
	   url:"${ctx}/base/client/getClientAll?filter_EQS_clientSort=0&setid=${bisForecast.clientId}",
	   valueField: 'ids',
	   textField: 'clientName',
	   mode:'remote',
	   onChange:function(){
...
//清空
function clearIt(){
	window.parent.mainpage.mainTabs.addModule('入库预报单管理','wms/forecast/manager');
}
...
function gridDG(forId){
   dg =$('#dg').datagrid({    
		method: "get",
	    url:'${ctx}/wms/forecastinfo/json/'+forId, 
	    fit : true,
		fitColumns : true,
		border : false,
		sortOrder:'desc',
...
function deleteIt(){
		...
		$.ajax({
				async:false,
				type: 'get',
				url: "${ctx}/wms/forecast/ifsave/" + forId,
		...
		parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
			if(data){
				$.ajax({
					async:false,
					type: 'get',
					url: "${ctx}/wms/forecast/delete/" + forId,
					success: function(data){
						if(data=="success"){
...
function addInfoT(){
	...
	d=$("#dlg").dialog({   
	    ...
	    width: 380,    
	    height: 380,    
	    href:'${ctx}/wms/forecastinfo/create/'+forId,
	    maximizable:true,
	    modal:true,
	    buttons:[{
			text:'确认',
...
function editInfo(){
	...
	d=$("#dlg").dialog({   
	    ...
	    width: 380,    
	    height: 380,    
	    href:'${ctx}/wms/forecastinfo/update/'+row.id,
	    maximizable:true,
	    modal:true,
	    buttons:[{
			text:'确认',
...
function delInfo(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type: 'get',
				url: "${ctx}/wms/forecastinfo/deleteinfo/" + ids,
				success: function(data){
...
d=$("#dlg").dialog({   
	    ...
	    width: 380,    
	    height: 340,    
	    href:'${ctx}/wms/forecastinfo/copy/'+row.id,
	    maximizable:true,
	    modal:true,
	    buttons:[{
			text:'增加',
	...
	$.ajax({
		...
		success: function(data){
			if(data != "success"){
				...
				$.ajax({
					type: 'get',
					url: "${ctx}/wms/forecast/customs/" + forId,
					success: function(data){
						if(data == "success"){
							successTip(data, dg);
						}else{
							parent.$.messager.show({title: "提示", msg: "入库报关单中存在此提单号数据！生成报关单失败！", position: "bottomRight" });
							return;
						}
					}
				});
	...
	$.ajax({
		...
		success: function(data){
			if(data != "success"){
				...
				$.ajax({
					type: 'get',
					url: "${ctx}/wms/forecast/ciq/" + forId,
					success: function(data){
						if(data == "success"){
							successTip(data, dg);
						}else{
							parent.$.messager.show({title: "提示", msg: "入库报检单中存在此提单号数据！生成报检单失败！", position: "bottomRight" });
							return;
						}
					}
				});
