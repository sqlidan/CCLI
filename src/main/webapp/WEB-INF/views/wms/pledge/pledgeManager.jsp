...
<div id="tt" class="easyui-tabs">   
    <div title="动态" data-options="refreshable: false">   
        <div id="tb" style="padding:5px;height:auto">
        	...
        	<select id="warehouseIdP" name="warehouseIdP" class="easyui-combobox" data-options="width: 180"  >
			</select>
		...
		<td><input id="zy" name="zy" value="质押" class="easyui-linkbutton" onclick="zy()" /> </td>
		</tr>
		</div>  
    </div>   
    <div title="静态" data-options="refreshable: false">   
    	 <div id="tb" style="padding:5px;height:auto">
        	...
        	<select id="warehouseIdjP" name="warehouseIdj" class="easyui-combobox" data-options="width: 180"  >
			</select>
		...
		<div>
		 ...
		 <td><input id="zy" name="zy" value="质押" class="easyui-linkbutton" onclick="zyj()" /></td>
		 </tr>
		 </div>  
...
function cx(){
    ...
    $.ajax({
      type: 'POST',
      url: "${ctx}/wms/pledge/gettraybysku",
...
function zy(){
	...
	if(zyPiece == ""){
		parent.$.messager.show({ title : "提示",msg: "请输入质押件数！", position: "bottomRight" });
		return;
	}
	if(warehouseId == ""){
		parent.$.messager.show({ title : "提示",msg: "请选择仓库！", position: "bottomRight" });
	...
	if($("#zyPiece").val() == '0'){
		parent.$.messager.show({ title : "提示",msg: "质押数量不得为0！", position: "bottomRight" });
		return;
	}
	if( parseInt(zyPiece) > parseInt(piece)){
		parent.$.messager.show({ title : "提示",msg: "质押件数不得大于可用件数！", position: "bottomRight" });
		return;
	}
	//后台校验件数
	$.ajax({
	  async: false,
	  type: 'POST',
	  url:"${ctx}/wms/pledge/ajaxd",
	  data:{"sku":sku,"clientId":clientId,"piece":piece,"warehouseId":warehouseId},
	  dataType: 'text',
	  success: function(msg){
	    if(msg != "success"){
	    	parent.$.messager.show({ title : "提示",msg: "因可用数量发生变化，质押失败！请重新查询后再尝试质押！", position: "bottomRight" });
	    	return;
	    }
	  }
	})
...
function cxj(){
    ...
    $.ajax({
      type: 'POST',
      url: "${ctx}/wms/pledge/gettraybystay",
...
function zyj(){
	...
	if(zyPiece == ""){
		parent.$.messager.show({ title : "提示",msg: "请输入质押数量！", position: "bottomRight" });
		return;
	}
	if(billNum == ""){
		parent.$.messager.show({ title : "提示",msg: "请输入提单号！", position: "bottomRight" });
	...
	if(parseInt(zyPiece) > parseInt(piece)){
		parent.$.messager.show({ title : "提示",msg: "质押数量不得大于可用数量！", position: "bottomRight" });
		return;
	}
	//后台校验件数
	$.ajax({
	...
	$.ajax({
	  async : false,
      type: 'POST',
      url: "${ctx}/wms/pledge/ifdong",
      data:{"sku":sku,"clientId":clientId,"warehouseId":warehouseId},
      dataType:'text',
      success: function(msg){
    	    if(msg == "success"){
    	    parent.$.messager.show({ title : "提示",msg: "已做过动态质押，无法进行静态质押操作！", position: "bottomRight" });
    	  	return;
    	  	}else{
    	  		$.ajax({
	 				async : false,
     		   		type: 'POST',
      				url: "${ctx}/wms/pledge/zyj",
