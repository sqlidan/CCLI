...
<div data-options="region:'center'" title="入库报关单总览" style="overflow-y:auto">
  ...
  <!--    	<select id="searchStock" name="filter_LIKES_clientName" class="easyui-combobox" data-options="width:150,prompt: '客户名称'" >
		...
		<shiro:hasPermission name="wms:customs:add">
	      	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-add" plain="true" onclick="window.parent.mainpage.mainTabs.addModule('入库报关管理','wms/customs/add')">添加</a>
	      	<span class="toolbar-item dialog-tool-separator"></span>
      	</shiro:hasPermission>
      	<shiro:hasPermission name="wms:customs:delete">
      		 <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" data-options="disabled:false" onclick="del()">删除</a>
       		<span class="toolbar-item dialog-tool-separator"></span>
      	</shiro:hasPermission>
      	<shiro:hasPermission name="wms:customs:update">
...
function info(id){
	dgg=$('#dgg').datagrid({    
		method: "get",
	    url:'${ctx}/wms/customsinfo/json/'+id, 
	    fit : true,
		fitColumns : true,
		border : false,
		idField : 'forId',
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/wms/customs/delete/"+row.id,
...
function update(){
	var row = dg.datagrid('getSelected');
	if(rowIsNull(row)) return;
		window.parent.mainpage.mainTabs.addModule('入库报关单修改','wms/customs/update/' + row.id);
}
...
function exportExcel(){
 	var url = "${ctx}/wms/customs/export";
...
function extension(){
	...
	d=$("#dlg").dialog({   
	    ...
	    width: 380,    
	    height: 380,    
	    href:'${ctx}/wms/customs/extension/'+row.cdNum,
	    maximizable:true,
	    modal:true,
	    buttons:[{
			text:'确认',
			handler:function(){
				if( $("#itemNum").val() =="" ) {
					parent.$.easyui.messager.show({title: "操作提示", msg: "请输入项号！", position: "bottomRight"});	
					return;
				}
