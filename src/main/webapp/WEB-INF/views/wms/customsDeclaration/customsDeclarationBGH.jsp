...
<div id="tb" style="padding:5px;height:auto">
	<div>
		...
		<shiro:hasPermission name="wms:customsDeclarationBGH:submit">
			<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="submit()">提交审核</a>
			<span class="toolbar-item dialog-tool-separator"></span>
		</shiro:hasPermission>
		<shiro:hasPermission name="wms:customsDeclarationBGH:uploadBGD">
...
function update(){
	...
	if(parseInt(row.state) > 0){
		parent.$.messager.show({ title : "提示",msg: "报关单信息已完善提交，不能再次修改。", position: "bottomRight" });
		return;
	}
	window.parent.mainpage.mainTabs.addModule('报关单编辑','wms/customsDeclaration/updateBGH/' + row.forId);
}
//提交审核
function submit(){
	var row = dg.datagrid('getSelected');
...
function declarationCheckList(){
	...
	parent.$.messager.confirm('提示', '您确定要将选中的报关单信息进行申报核注清单吗？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/wms/customsDeclaration/UpdateState/"+row.forId+"/bghDeclarationCheckList",
				success: function(data){
					successTip(data,dg);
				},
			});
