        ...
        <div data-options="region:'center'">
            <form id="mainForm">
                <div style="height:auto" class="datagrid-toolbar">
                    <shiro:hasPermission name="wms:customsDeclarationBGH:edit">
...
<%--                    <shiro:hasPermission name="wms:customsDeclarationBGH:edit">--%>
<%--                        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-disk" plain="true"--%>
<%--                           data-options="disabled:false" onclick="clearIt()">清空</a>--%>
<%--                        <span class="toolbar-item dialog-tool-separator"></span>--%>
<%--                    </shiro:hasPermission>--%>
                </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" id="spbh" name="spbh" class="easyui-validatebox"
                                   data-options="width:180, required:'required'" value="${bisPreEntryInfo.spbh}">
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
                            <select id="ycg" name="ycg" class="easyui-combobox"
                                    data-options="width:180, required:'required'" value="${bisPreEntryInfo.ycg}">
                            </select>
                        </td>
...
<script type="text/javascript">
    ...
    function submitForm() {
        if ($("#mainForm").form('validate')) {
            ...
            $.ajax({
                async: false,
                type: 'POST',
                url: "${ctx}/wms/customsDeclaration/updateBGH",
    ...
    function addDJInfo() {
        dt = $("#dlg").dialog({
            ...
            width: 500,
            height: 300,
            href: '${ctx}/wms/customsDeclarationInfo/createDJ/' + forId,
            maximizable: true,
            modal: true,
            buttons: [{
                text: '确认',
    ...
    function delDJInfo() {
        ...
        parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function (data) {
            if (data) {
                $.ajax({
                    type: 'get',
                    url: "${ctx}/wms/customsDeclarationInfo/deleteDJinfo/" + ids,
    ...
    function submit() {
        parent.$.messager.confirm('提示', '您确定要申报提交吗？', function (data) {
            if (data) {
                $.ajax({
                    type: 'get',
                    url: "${ctx}/wms/customsDeclarationInfo/submitDJinfo/" + forId,
                    success: function (data) {
                        dg.datagrid('clearSelections');
                        successTip(data, dg);
                    }
    ...
    function addInfo() {
        if ($("#mainForm2").form('validate')) {
            ...
            $.ajax({
                async: false,
                type: 'POST',
                url: "${ctx}/wms/customsDeclarationInfo/create/" + forId,
