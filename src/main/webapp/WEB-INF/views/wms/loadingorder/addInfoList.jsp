			  ...
			  if (rows!=null && rows.length>0) {
	            	...
	            	$(rows).each(function(i){
	            		...
	            		$('#infodg').datagrid('updateRow',{
                    		index:i,
                    		row: {
                    			PIECE: '<input type=\"text\" id=\"piece'+i+'\" name=\"piece\" style=\"width:100%\" value=\"'+getVal+'\" onchange=\"checkPice(this.value,'+row["NOW_PIECE"]+','+row["TNUM"]+','+i+',\''+row["LAB"]+'\')\" onkeyup=\"value=value.replace(\/[^\\-?\\d]/g,\'\')\" >',
                    			NUM:i,
                    			RK_NUM:rk
                    		}
                    	});
...
//判断 输入的出库件数
function checkPice(val,sum,tsum,i,lab){
	if(val!=""){
