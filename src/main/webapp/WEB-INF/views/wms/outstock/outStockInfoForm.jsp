	...
	function grid(){
		...
		$('#infodg').datagrid({
			...
			columns:[[
				...
				{field:'PLEDGE_PIECE',title:'大宗质押件数',sortable:true,width:80},
...
//	            			getSale=getSalesNum(inforows,row);
						...
						$('#infodg').datagrid('updateRow',{
							index:i,
							row: {
								PIECE: '<input type=\"text\" id=\"piece'+i+'\" name=\"piece\" style=\"width:100%\" value=\"'+getVal+'\"  onchange=\"checkPice(this.value,'+row["PLEDGE_PIECE"]+','+row["NOW_PIECE"]+','+row["HAS"]+','+row["TNUM"]+','+row["TTNUM"]+','+i+',\''+row["LAB"]+'\')\"  onkeyup=\"value=value.replace(\/[^\\-?\\d]/g,\'\')\" >',
								NUM:i,
	...
	//判断 输入的出库件数
	function checkPice(val,PLEDGE_PIECE,sum,has,tsum,ttsum,i,lab){
		if(val!=""){
			// 现有件数 《 出库件数+质押件数+已添加件数+静态质押件数+动态质押件数
			if(Number(sum)<(Number(tsum)+Number(val)+Number(ttsum)+Number(has)+Number(PLEDGE_PIECE))){
				$("#piece"+i).val("");
