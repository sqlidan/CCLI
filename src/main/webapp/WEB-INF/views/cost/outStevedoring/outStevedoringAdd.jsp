...
<body>
<div>
	<form id="mainform" action="${ctx}/cost/outstevedoring/${action}" method="post">
		<table class="formTable">
			<tr>
				<td>装车号：</td>
				<td> 
				<input type="hidden" id="droLoadingTime" name="droLoadingTime" value="">
				<input type="hidden" id="droStockIdName" name="droStockIdName" value="">
				<input type="hidden" id="droStockId" name="droStockId" value="">
				
					<input type="hidden" id="netW" name="netWeight" value="">
					<input id="loadingNume" name="loadingNum" class="easyui-validatebox" data-options="width: 150, required:'required'" value="" readonly> 
				</td>
			</tr>
			<tr>
				<td>装卸队：</td>
...
$("#loadingNume").val( $("#loadingTruckNum").val() ); 
$("#droLoadingTime").val( $("#loadingTime").val() );
$("#droStockIdName").val( $("#stockId").val() );
$("#droStockId").val( $("#stockIdName").val() );
$("input[id$='Numa']").val(0);
//客户
	   $('#clientId').combobox({
	   method:"GET",
