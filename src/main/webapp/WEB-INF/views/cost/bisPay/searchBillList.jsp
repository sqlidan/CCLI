...
$(document).ready(function(){
	dglx=$('#dglx').datagrid({    
	    ...
	    columns:[[    
    		...
    		{field:'STOCK_ORG_ID',title:'结算方id',hidden:true},
 	        {field:'STOCK_ORG',title:'结算方',sortable:true,width:100},
 	        {field:'STOCK_ID',title:'存id',hidden:true},
 	        {field:'STOCK',title:'存货方',sortable:true,width:100},
 	        {field:'RK_TIME',title:'入库时间',sortable:true,width:100},
...
function addLinkTBRow(jsonObj){
	...
	$('#dglx').datagrid('insertRow',{
		...
		row: {
			...
			NTYPE:jsonObj.NTYPE,
			STOCK:jsonObj.STOCK,
			STOCK_ID:jsonObj.STOCKID,
			RK_TIME:jsonObj.RK_TIME,
			ITEMNUM:jsonObj.ITEMNUM,
			STOCKID:jsonObj.STOCKID,
			STOCK_ORG_ID:jsonObj.STOCK_ORG_ID,
			STOCK_ORG:jsonObj.STOCK_ORG
		}
	});
