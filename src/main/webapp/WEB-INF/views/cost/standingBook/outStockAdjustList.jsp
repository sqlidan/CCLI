...
<div data-options="region:'north', split:true, border:false" style="height:100px;">
	<div class="datagrid-toolbar" style="height:auto">
	    ...
	    <shiro:hasPermission name="wms:outStockFee:backPass">
		    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-user-edit" plain="true" onclick="isNotOK()">取消审核</a>
		    <span class="toolbar-item dialog-tool-separator"></span>
	    </shiro:hasPermission>
	    <c:choose>
	    	<c:when test="${clear == '1'}">
	    		  <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-user-edit" plain="true" onclick="clearance()">清库结算</a>
	    		 <span class="toolbar-item dialog-tool-separator"></span>
	   		</c:when>
	   	</c:choose>
	    <c:choose>
				<c:when test= "${finishFeeState == '0' && planFeeState == '0'}" >
...
//                         editable: false, //不允许手动输入
	...
	dg2 = $('#dg2').datagrid({    
	    ...
	    columns:[[    
	        ...
	        {field: 'payNum', title: '业务付款单号',width: 100},
	        {field: 'reconcileNum', title: '对账单号',width: 100},
	        {field:'billNum',title:'提单号', width:100,
	        	editor:{
	        		type:'combobox',
                    options:{
                    	onBeforeLoad: function(param){
...
function deletePaySign(standingNum, examineSign){
	if(examineSign == 1){
		parent.$.messager.show({title: "提示", msg: "审核后不得取消垫付！", position: "bottomRight"});
		dg2.datagrid('reload');
	}else{
				...
				$.ajax({
					...
					success: function(data){
						if(data == 'success'){
							...
							parent.$.messager.show({title: "提示", msg: "应收应付审核后不得取消垫付！", position: "bottomRight"});
						}
						dg1.datagrid('reload');
						dg2.datagrid('reload');
						sunCost();
...
function openShare(){
	...
	d1 = $("#dlg1").dialog({   
	    ...
	    buttons:[{
			...
			text:'撤销分摊',
			handler:function(){
				parent.$.messager.confirm('提示', '确定要撤销分摊吗？<span style="color:red">已分摊的费目会被删除！</span>', function(data){
					if (data){
						$.ajax({
							async: false,
							type: 'POST',
							contentType: 'application/json;charset=utf-8',
							url: "${ctx}/base/share/cancelCostShare/"+linkId+"/"+stockId,
							success: function(data){
								if(data == "success"){
									parent.$.messager.show({title: "提示", msg: "操作成功！", position: "bottomRight" });
								}
...
function planOk(){
	...
	$.ajax({
				type:'get',
				url:"${ctx}/wms/outstock/planok/"+'${linkId}',
				success: function(data){
					parent.$.messager.show({ title : "提示",msg: "操作成功！", position: "bottomRight" });
...
function planNo(){
	...
	$.ajax({
				type:'get',
				url:"${ctx}/wms/outstock/planno/"+'${linkId}',
				success: function(data){
					parent.$.messager.show({ title : "提示",msg: "操作成功！", position: "bottomRight" });
					window.parent.mainpage.mainTabs.refCurrentTab();//刷新TAB
				}
  ...
  function finishOk(){
	...
	$.ajax({
				type:'get',
				url:"${ctx}/wms/outstock/finishok/"+'${linkId}',
				success: function(data){
					parent.$.messager.show({ title : "提示",msg: "操作成功！", position: "bottomRight" });
					window.parent.mainpage.mainTabs.refCurrentTab();//刷新TAB
				}
...
function finishNo(){
	...
	$.ajax({
				type:'get',
				url:"${ctx}/wms/outstock/finishno/"+'${linkId}',
				success: function(data){
					parent.$.messager.show({ title : "提示",msg: "操作成功！", position: "bottomRight" });
					window.parent.mainpage.mainTabs.refCurrentTab();//刷新TAB
				}
