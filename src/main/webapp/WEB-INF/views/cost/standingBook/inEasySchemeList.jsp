        ...
        <div>
        	<form id="searchFromEasy" action="">
        		<input type="hidden" id="linkId" name="linkId" value="${linkId }">
       	        ...
       	        <select class="easyui-combobox" id="clientId" name="filter_EQS_customsId" data-options="width:150, prompt: '客户名称'">
				</select>
			...
			<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="addScheme()">引入</a>
       		<span class="toolbar-item dialog-tool-separator"></span>
        </div> 
  </div>
<table id="dg"></table> 
...
<script type="text/javascript">
	...
	document.onkeydown = function () {if(event.keyCode == 13){cxc();}};
...
$(function(){
	$('#ifGet').combobox({    
	    onChange: function(newValue, oldValue){
			...
			for(var i=0; i < datas.length; i++){
				$('#dg').datagrid("unselectRow", datas[i].SCHEME_NUM);
			}
	    	
	    	if(newValue == "2"){
	    		$('#clientId').combobox("select", "");
				$('#clientId').combobox('enable');
				cxc();
	    	}else{
	    		var clientIdt = '${client.ids}';
	    		$('#clientId').combobox({
    		       disabled: true,
    			   method: "GET",
    			   url: "${ctx}/base/client/getClientAll?setid=${client.ids}",
    			   valueField: 'ids',
    			   textField: 'clientName',
    			   mode: 'remote',
    			   onLoadSuccess:function(){
    			   	   if(clientIdt != null && clientIdt !=""){
    				   		$('#clientId').combobox("select", clientIdt);
    				   		clientIdt="";
    				   		cxc();
    				   }
    				}
	...
	dg = $('#dg').datagrid({    
	    ...
	    columns:[[    
	        ...
	        {field:'PROGRAM_TYPE',title:'方案类型',hidden:true,sortable:true,width:100,
	        ...
	        {field: 'BIS_TYPE', title:'业务类型', hidden:true,sortable:true, width:100,
	        	formatter: function(value, row, index) {
					if (value == 1) {
						return '1';
					}
	...
	window.setTimeout(function(){cxc()},500);
	...
	parent.$.messager.confirm('提示', '确认添加？', function(data){
		if (data){
			...
			$.ajax({
				...
				url: "${ctx}/cost/standingBook/"+ linkId +"/addInStandingSchemeBatch",
