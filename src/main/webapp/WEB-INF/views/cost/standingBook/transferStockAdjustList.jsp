...
<!-- 	  	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-hamburg-sign-up" plain="true" data-options="disabled:false" onclick="add()">引入方案</a> -->
	    ...
	    <shiro:hasPermission name="wms:transferStockFee:okPass">
		    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-user-edit" plain="true" onclick="isOK()">审核</a>
		   	<span class="toolbar-item dialog-tool-separator"></span>
	   	</shiro:hasPermission>
	   	<shiro:hasPermission name="wms:transferStockFee:backPass">
		   	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-user-edit" plain="true" onclick="isNotOK()">取消审核</a>
		   	<span class="toolbar-item dialog-tool-separator"></span>
		</shiro:hasPermission>
<!-- 	   	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-hamburg-customers" plain="true" onclick="openShare()">费用分摊</a> -->
<!-- 	   	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-hamburg-cost" plain="true" onclick="">清库结算</a> -->
<!-- 	   	<span class="toolbar-item dialog-tool-separator"></span> -->
...
$(function(){   
	...
	dg1 = $('#dg1').datagrid({    
	    ...
	    columns:[[    
	        ...
	        {field:'reconcileSign',title:'对账',align:'center',  width:30,
	        	formatter : function(value, row, index) {
	         		if(value == 0){
	        	    	return '<input type="checkbox" readonly disabled/>';
	        	    }
	        ...
	        {field:'settleSign',title:'结算',align:'center',  width:30,
	        	formatter : function(value, row, index) {
	         		if(value == 0){
	        	    	return '<input type="checkbox" readonly disabled/>';
	        	    }
	        ...
	        {field:'chargeSign',title:'收款',align:'center',  width:30,
	        	formatter : function(value, row, index) {
	         		if(value == 0){
	        	    	return '<input type="checkbox" readonly disabled/>';
	        	    }
	        ...
	        {field:'paySign',title:'垫付',align:'center',  width:30,
	        	formatter: function(value, row, index) {
	         		if(value == 0){
	        	    	return '<input type="checkbox" readonly disabled/>';
	        	    }
	        ...
	        {field:'asn',title:'ASN',  width:30, editor: "text"},
	        {field:'billNum',title:'提单号',  width:50,
	        	editor:{
	        		type:'combobox',
                    options:{
                    	...
                    	url: "${ctx}/wms/outstockinfo/getOutStockInfoByLinkId",
                    	valueField: 'billNum',
                        textField: 'billNum',
                        mode: 'remote',
//                         editable: false, //不允许手动输入
	        ...
	        {field:'feeCode',title:'费用名称',  width:50,
	        	formatter: function(value, row, index) {
	       			return row.feeName;
	        	},
	        	editor:{
	        ...
	        {field:'customsNum',title:'客户',  width:200,
	        	formatter: function(value, row, index) {
	        ...
	        {field:'num',title:'数量', width:50, editor: {type:'numberbox',options:{precision:2}}},
	        ...
	        {field:'exchangeRate',title:'汇率',  width:50},
	        {field:'taxRate',title:'税率', width:50, editor: "numberbox",
	        	editor:{
	        		type:'combobox',
                    options:{
                    	data:[
	        ...
	        {field:'billDate',title:'账单日期',  width:50, editor: "my97M"},
	        ...
	        {field:'remark',title:'备注',  width:50, editor: "text"},
	        {field:'inputPersonId',title:'inputPersonId',hidden:true},
	        {field:'inputPerson',title:'创建人', width:50},
	        {field:'inputDate',title:'创建时间', width:50},
	        {field:'updatePerson',title:'修改人', width:50},
	        {field:'updateDate',title:'修改时间', width:50}
	    ]],
...
//                         editable: false, //不允许手动输入
	        ...
	        {field:'billDate',title:'账单日期',  width:50, editor: "my97"},
	        {field:'fillSign',title:'冲补', align:'center', width:50,
	        	formatter : function(value, row, index) {
	         		if(value == 0){
	        	    	return 'N';
		...
		parent.$.messager.confirm('提示', '确定要修改费目信息？', function(data){
			if (data){
				...
				$.ajax({
					...
					data: "crkSign=2&type=1&stockId=" + rowData.customsNum + "&billNum=" + rowData.billNum + "&asn=" + rowData.asn + "&num="+ rowData.num + "&price=" + rowData.price + "&taxRate="+ rowData.taxRate
		...
		parent.$.messager.confirm('提示', '确定要修改费目信息？', function(data){
			if (data){
				...
				$.ajax({
					...
					async: false,
					url: "${ctx}/cost/standingBook/"+ rowData.standingNum + "/"+ linkId +"/editStandingBookByFee",
					data: "crkSign=2&type=2&stockId=" + rowData.customsNum + "&billNum=" + rowData.billNum + "&asn=" + rowData.asn + "&num="+ rowData.num + "&price=" + rowData.price + "&taxRate="+ rowData.taxRate
						+ "&feeCode=" + rowData.feeCode+ "&currency="+rowData.currency+ "&billDate="+rowData.billDate+ "&remark="+rowData.remark,
					success: function(data){
						if(data == 'success'){
							parent.$.messager.show({title: "提示", msg: "操作成功！", position: "bottomRight"});
...
function add() {
	...
	d1 = $("#dlg1").dialog({   
	    ...
	    width: 1000,    
	    height: 420,    
	    href: '${ctx}/base/scheme/outEasySchemeList/'+ stockId + "/" + linkId,
	    maximizable: true,
	    modal: true
	});
}
...
function del(){
	...
	if(row1 == null && row2 == null){
		...
		parent.$.messager.confirm('提示', '其中审核通过的删除无效！', function(data){
			if (data){
				var userId = $("#userId").val();
				
				var newIdsList = [];
				var datas1 = dg1.treegrid('getSelections');
				for(var i=0; i < datas1.length; i++){
					if(datas1[i].examineSign != 1 && datas1[i].inputPersonId == userId){
						newIdsList.push(datas1[i].standingNum);
					}
				}
				
				var datas2 = dg2.treegrid('getSelections');
