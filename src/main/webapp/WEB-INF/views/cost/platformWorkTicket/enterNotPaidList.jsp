...
$(function(){   
	dg=$('#dg').datagrid({    
	method: "get",
    url:'${ctx}/cost/enterNotPaid/page',
    fit : true,
	fitColumns : true,
	border : false,
	idField : 'ids',
    ...
    columns:[[    
		...
		{field:'asnTransNum',title:'ASN',sortable:true,width:100},

		{field:'containerNo',title:'箱号',sortable:false,width:100},
        {field:'platNo',title:'车号',sortable:true,width:100},
		{field:'inWeight',title:'重量',sortable:true,width:100},
...
function getEnterWeight(){
	$.ajax({
		async: false,
		type: "POST",
		url: "${ctx}/cost/platformWorkTicket/enterWeight",
		data: {
...
function generateCost(){
	...
	window.parent.mainpage.mainTabs.addModule('入库装卸管理','cost/enterstevedoring/update/' + row.asnTransNum);
// window.parent.mainpage.mainTabs.addModule('入库装卸管理','cost/enterstevedoring/list/');
}

//生成费用 和修改传参一致
function generateCost1(){
	var row = dg.datagrid('getSelected');
	if(row == null) {
		parent.$.messager.show({title: "提示", msg: "请选择行数据！", position: "bottomRight" });
		return;
...
function add(){
	...
	if(asnNum==''||asnNum==null){
		parent.$.messager.show({title: "提示", msg: "asn为空！", position: "bottomRight" });
		return;
	}
	//判断是否有相应的费用方案
	d=$("#dlg").dialog({
		...
		width: 480,
		height: 480,
		href:'${ctx}/cost/enterNotPaid/add/'+row.id,
		maximizable:true,
		modal:true,
		buttons:[{
			text:'确认',
