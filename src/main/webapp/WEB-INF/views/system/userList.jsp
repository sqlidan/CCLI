...
<div id="tb" style="padding:5px;height:auto">
        <div>
        	<form id="searchFrom" action="">
       	        ...
       	        <input type="text" name="filter_LIKES_workNo" class="easyui-validatebox" data-options="width:150,prompt: '工号'"/>
        	...
        	<shiro:hasPermission name="sys:user:reset">
        		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cologne-home" plain="true" onclick="resetPassWord()">重置密码</a>
        	</shiro:hasPermission>
        	<shiro:hasPermission name="sys:user:updPassword">
        		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cologne-home" plain="true" onclick="updPassWord()">修改密码</a>
			</shiro:hasPermission>
...
$(function(){   
    ...
    columns:[[    
        ...
        {field:'userCode',title:'编码',sortable:true,width:100},
        {field:'name',title:'姓名',sortable:true,width:100},
        {field:'workNo',title:'工号',sortable:true,width:100},
...
function resetPassWord(){
	...
	if(rowIsNull(row)) return;
	 $.ajax({
				...
				success: function(data){
					if(data=="success"){
						parent.$.messager.show({ title : "提示",msg: "密码重置成功！", position: "bottomRight" });
					}else{
						parent.$.messager.show({ title : "重置失败！" });
					}
				}
			});
}
...
function exportExcel(){
	var url = "${ctx}/system/user/exportExcel";
