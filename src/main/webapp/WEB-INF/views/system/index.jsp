...
<!--                     <h1 style="margin-left: 10px; margin-top: 10px;color: #fff">CCLI<span style="color: #3F4752">管理系统</span></h1> -->
<!--                     <h1 style="margin-left: 10px; margin-top: 10px;color: #fff">怡之航冷链物流综合管理系统</h1> -->
<%--                     <img src="${ctx}/static/images/system_name.png"/> --%>
                         <img src="${ctx}/static/images/title.png"/>
            ...
            <div id="toolbar" class="panel-header panel-header-noborder top-toolbar">
                ...
                <div id="buttonbar">
                    ...
                    <a href="javascript:void(0);" class="easyui-menubutton" data-options="menu:'#layout_north_set'" iconCls="icon-standard-cog">系统
<%--                     	<spring:message code="System"/> --%>
                    </a>  
                    <div id="layout_north_set">
						<div id="btnFullScreen" data-options="iconCls:'key'">全屏切换</div>
						<div id="updatePwd" data-options="iconCls:'key'" onclick="updatePwd()">修改密码</div>
...
//检查是否存在单点登录的cookie,如果不存在跳回登录页面
function checkCookie(){
	isExistToken=document.cookie.indexOf("x-access-token"); 
	console.log("LogoutisExistToken:::"+isExistToken);
	if(isExistToken== -1){
...
function outWarning(){
	$.ajax({
		async:false,
		type:'get',
		url:"${ctx}/wms/extension/outwarning",
		success: function(data){
			if(data != "success"){
				alert("如下提单号对应的关检单已到展期预警时间，请及时处理: "+ data + "!");
			} 
		}
	});
	window.setTimeout(function(){outWarning()},1800000);
}
...
function taskRemind(){
	...
	window.setTimeout(function(){taskRemind()},1800000);
}
